#ifndef WSS_STATUS_CODE
#define WSS_STATUS_CODE
...
inline StatusCode status_code(const std::string &status_code_str) noexcept {
  class StringToStatusCode : public std::unordered_map<std::string, wss::server::StatusCode> {
   public:
      StringToStatusCode() {
        for (auto &status_code : wss::server::status_codes())
...
const inline std::string &status_code(StatusCode status_code_enum) noexcept {
  class StatusCodeToString : public std::map<wss::server::StatusCode, std::string> {
   public:
      StatusCodeToString() {
        for (auto &status_code : wss::server::status_codes())
