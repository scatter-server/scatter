...
class ClientBase {
 ...
 public:
    ...
    void request(const std::string &method,
        ...
        auto request_callback =
            std::make_shared<std::function<void(std::shared_ptr<Response>, const error_code &)>>(std::move(
                request_callback_));
        session->callback =
 ...
 protected:
    ...
    std::shared_ptr<Connection> get_connection() noexcept {
        ...
        if (!query) {
            if (config.proxy_server.empty())
                query = std::unique_ptr<asio::ip::tcp::resolver::query>(new asio::ip::tcp::resolver::query(host,
                                                                                                           std::to_string(
                                                                                                               port)));
            else {
                auto proxy_host_port = parse_host_port(config.proxy_server, 8080);
                query =
                    std::unique_ptr<asio::ip::tcp::resolver::query>(new asio::ip::tcp::resolver::query(proxy_host_port
                                                                                                           .first,
                                                                                                       std::to_string(
                                                                                                           proxy_host_port
                                                                                                               .second)));
    ...
    std::unique_ptr<asio::streambuf> create_request_header(const std::string &method,
                                                           const std::string &path,
    ...
    std::pair<std::string, unsigned short> parse_host_port(const std::string &host_port,
    ...
    void read(const std::shared_ptr<Session> &session) {
        ...
        asio::async_read_until(*session->connection->socket,
                               ...
                               [this, session](const error_code &ec, std::size_t bytes_transferred) {
                                 ...
                                 if (!lock)
                                     return;
                                 if ((!ec || ec == asio::error::not_found) && session->response->streambuf.size()
                                 ...
                                 if (!ec) {
                                     ...
                                     std::size_t
                                         num_additional_bytes = session->response->streambuf.size() - bytes_transferred;
    ...
    void read_chunked(const std::shared_ptr<Session> &session, const std::shared_ptr<asio::streambuf> &tmp_streambuf) {
        ...
        asio::async_read_until(*session->connection->socket,
                               *chunked_streambuf,
                               "\r\n",
                               [this, session, chunked_streambuf, tmp_streambuf](const error_code &ec,
                                                                                 std::size_t bytes_transferred) {
                                 session->connection->cancel_timeout();
                                 auto lock = session->connection->handler_runner->continue_lock();
                                 if (!lock)
                                 ...
                                 if ((!ec || ec == asio::error::not_found)
                                     && chunked_streambuf->size() == chunked_streambuf->max_size()) {
                                     session->callback(session->connection,
                                                       make_error_code::make_error_code(errc::message_size));
                                     return;
                                 }
                                     ...
                                     if ((2 + length) > num_additional_bytes) {
                                         ...
                                         asio::async_read(*session->connection->socket,
                                                          *chunked_streambuf,
                                                          ...
                                                          [this, session, chunked_streambuf, post_process](const error_code &ec,
                                                                                                           std::size_t /*bytes_transferred*/) {
                                                            session->connection->cancel_timeout();
                                                            auto lock =
                                                                session->connection->handler_runner->continue_lock();
                                                            if (!lock)
                                                                return;
                                                            if (!ec) {
                                                                if (chunked_streambuf->size()
                                                                    == chunked_streambuf->max_size()) {
                                                                    session->callback(session->connection,
                                                                                      make_error_code::make_error_code(
                                                                                          errc::message_size));
                                                                    return;
        ...
        if (!session->connection->socket->lowest_layer().is_open()) {
            ...
            resolver->async_resolve(*query,
                                    [this, session, resolver](const error_code &ec,
                                      ...
                                      if (!ec) {
                                          session->connection->set_timeout(config.timeout_connect);
                                          asio::async_connect(*session->connection->socket,
                                                              it,
                                                              [this, session, resolver](const error_code &ec,
                                                                                        asio::ip::tcp::resolver::iterator /*it*/) {
                                                                session->connection->cancel_timeout();
