 ...
 protected:
    class Connection : public std::enable_shared_from_this<Connection> {
     public:
        ...
        Connection(std::shared_ptr<ScopeRunner> handler_runner, Args &&... args) noexcept : handler_runner(std::move(
            handler_runner)), socket(new socket_type(std::forward<Args>(args)...)) { }
        ...
        asio::async_read_until(*session->connection->socket,
                               ...
                               [this, session](const error_code &ec, std::size_t bytes_transferred) {
                                 ...
                                 if (!ec) {
                                     ...
                                     std::size_t
                                         num_additional_bytes = session->request->streambuf.size() - bytes_transferred;
