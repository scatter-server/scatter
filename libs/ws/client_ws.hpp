        ...
        void send_from_queue() {
            ...
            strand.post([self]() {
              asio::async_write(*self->socket,
                                self->send_queue.begin()->send_stream->streambuf,
 ...
 protected:
    ...
    SocketClientBase(const std::string &host_port_path, unsigned short default_port) noexcept
        ...
        if (host_end == std::string::npos) {
            ...
            if (host_port_end == std::string::npos)
                ...
                port = static_cast<unsigned short>(stoul(host_port_path
                                                             .substr(host_end + 1, host_port_end - (host_end + 1))));
        }
        if (host_port_end == std::string::npos)
            path = "/";
        else
    ...
    void handshake(const std::shared_ptr<Connection> &connection) {
        ...
        asio::async_write(*connection->socket,
                          *write_buffer,
                          [this, connection, write_buffer, nonce_base64](const error_code &ec,
                            ...
                            if (!ec) {
                                ...
                                asio::async_read_until(*connection->socket,
                                                       connection->message->streambuf,
                                                       "\r\n\r\n",
                                                       [this, connection, nonce_base64](const error_code &ec,
                                                                                        std::size_t /*bytes_transferred*/) {
                                                         connection->cancel_timeout();
                                                         auto lock = connection->handler_runner->continue_lock();
                                                         if (!lock)
                                                             return;
                                                         if (!ec) {
                                                             if (!ResponseMessage::parse(*connection->message,
                                                                                         connection->http_version,
                                                                                         connection->status_code,
                                                                                         connection->header) ||
                                                                 connection->status_code.empty()
                                                                 || connection->status_code.compare(0, 4, "101 ")
                                                                     != 0) {
                                                                 this->connection_error(connection,
                                                                                        make_error_code::make_error_code(
                                                                                            errc::protocol_error));
                                                                 return;
                                                             }
                                                             auto header_it =
                                                                 connection->header.find("Sec-WebSocket-Accept");
                                                             static auto ws_magic_string =
                                                                 "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
                                                             if (header_it != connection->header.end() &&
                                                                 Crypto::Base64::decode(header_it->second)
                                                                     == Crypto::sha1(*nonce_base64 + ws_magic_string)) {
                         ...
                         [this, connection](const error_code &ec, std::size_t bytes_transferred) {
                           ...
                           if (!ec) {
                               if (bytes_transferred
                                   == 0) { // TODO: This might happen on server at least, might also happen here
                                   ...
                                   asio::async_read(*connection->socket,
                                                    ...
                                                    [this, connection](const error_code &ec,
                                                      ...
                                                      if (!ec) {
                                                          ...
                                                          connection->message
                                                                    ->read(reinterpret_cast<char *>(&length_bytes[0]),
                                                                           2);
    ...
    void read_message_content(const std::shared_ptr<Connection> &connection) {
        asio::async_read(*connection->socket,
                         connection->message->streambuf,
                         asio::transfer_exactly(connection->message->length),
                         [this, connection](const error_code &ec, std::size_t /*bytes_transferred*/) {
                           auto lock = connection->handler_runner->continue_lock();
                           if (!lock)
                               return;
...
class SocketClient<WS> : public SocketClientBase<WS> {
 public:
    SocketClient(const std::string &server_port_path) noexcept
        : SocketClientBase<WS>::SocketClientBase(server_port_path, 80) { };

 protected:
    void connect() override {
        std::unique_lock<std::mutex> lock(connection_mutex);
        auto connection = this->connection = std::shared_ptr<Connection>(new Connection(handler_runner,
                                                                                        config.timeout_idle,
                                                                                        *io_service));
        lock.unlock();
        ...
        resolver->async_resolve(query,
                                [this, connection, resolver](const error_code &ec,
