...
class SocketServerBase {
 public:
    ...
    class Connection : public std::enable_shared_from_this<Connection> {
     ...
     public:
        explicit Connection(std::unique_ptr<socket_type> &&socket) noexcept :
     ...
     private:
        ...
        void send_from_queue() {
            ...
            strand.post([self]() {
              asio::async_write(
                  *self->socket,
                  self->send_queue.begin()->header_stream->streambuf,
                  self->strand.wrap([self](const error_code &ec, std::size_t /*bytes_transferred*/) {
                    ...
                    if (!ec) {
                        // write handler
                        std::function<void(const error_code &, std::size_t /*bytes_transferred*/)> writeHandler;
                        writeHandler = [self](const error_code &ec, std::size_t) {
                          auto lock = self->handler_runner->continue_lock();
                          if (!lock) {
                              return;
                          }
 ...
 protected:
    ...
    /**
     * @TODO не собирает фрагменты сообщений, сделать буффер тут
     * @param connection
     * @param endpoint
     */
        ...
        asio::async_read(
            *connection->socket,
            ...
            [this, connection, &endpoint](const error_code &ec, std::size_t bytes_transferred) {
              auto lock = connection->handler_runner->continue_lock();
              if (!lock) {
                  std::cout << "--Locked read_message" << std::endl;
                  return;
              ...
              if (bytes_transferred == 0) { // TODO: why does this happen sometimes?
                  std::cerr << "Bytes transferred: 0. Trying again (for a while?) ??" << std::endl;
//                  read_message(connection, endpoint);
               ...
               *        это и есть размер
               *    2 байт == 126
               *        читаем следующие 2 байта, это будет наш размер
               *    2 байт == 127
               *        читаем следующие 8 байт, это будет наш размер
               *
               *
               *  логика
               *
              ...
              if (length == 126) {
                  ...
                  asio::async_read(
                      *connection->socket,
                      connection->read_buffer,
                      asio::transfer_exactly(2),
                      [this, connection, &endpoint, fin_rsv_opcode](const error_code &ec,
                                                                    std::size_t /*bytes_transferred*/) {
                        auto lock = connection->handler_runner->continue_lock();
                        if (!lock) {
                            std::cout << "--Locked read_message::126" << std::endl;
                            return;
                        }
                        ...
                        for (std::size_t c = 0; c < num_bytes; c++) {
                            sz += static_cast<std::size_t>(length_bytes[c]) << (8 * (num_bytes - 1 - c));
                        }

                        read_message_content(connection, sz, endpoint, fin_rsv_opcode);
                      });
              } else if (length == 127) {
                  ...
                  asio::async_read(
                      *connection->socket,
                      ...
                      [this, connection, &endpoint, fin_rsv_opcode](const error_code &ec,
                        ...
                        auto lock = connection->handler_runner->continue_lock();
                        if (!lock) {
                            std::cout << "--Locked read_message::127" << std::endl;
                            return;
                        }
        ...
        asio::async_read(
            *connection->socket,
            ...
            [this, connection, length, &endpoint, fin_rsv_opcode](const error_code &ec,
              ...
              auto lock = connection->handler_runner->continue_lock();
              if (!lock) {
                  std::cout << "--Locked read_message_content" << std::endl;
                  return;
              }
    ...
    void connection_close(const std::shared_ptr<Connection> &connection,
                          Endpoint &endpoint,
                          int status,
    ...
    void connection_error(const std::shared_ptr<Connection> &connection,
                          Endpoint &endpoint,
